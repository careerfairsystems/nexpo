defmodule Nexpo.Statistics do
  ...
  def get_all() do
    ...
    words_per_appl =
      Enum.reduce(applications, 0, fn x, acc -> length(String.split(x.motivation, " ")) + acc end) /
        length(applications)
      ...
      Repo.all(
        from(company in Company,
          ...
          select: %{
            id: company.id,
            name: company.name,
            nbr_applications: count(appl.id, :distinct),
            scored_applications: count(scored_appl.id, :distinct)
          }
        )
      )
